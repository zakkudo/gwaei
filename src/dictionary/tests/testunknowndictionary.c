...
void setup (Fixture *fixture, gconstpointer data)
{
  ...
  if (module == NULL)
  {
    module = lw_dictionarymodule_new (".." G_DIR_SEPARATOR_S ".libs" G_DIR_SEPARATOR_S "libunknowndictionary." G_MODULE_SUFFIX);
  }

  GType type = g_type_from_name ("LwUnknownDictionary");
...
{
  g_assert_cmpstr ("LwUnknownDictionary/radicalsdictionary.data", ==, lw_dictionary_get_id (fixture->dictionary));
}
...
{
  gchar * contents_path = g_build_filename (fixture->datadir, "libwaei", "dictionary", "LwUnknownDictionary", "test new filename", NULL);
  ...
  g_assert_cmpuint (0, ==, lw_dictionary_contents_length (fixture->dictionary));
  g_assert_null (lw_dictionary_get_contents_checksum (fixture->dictionary));
  g_assert_cmpstr ("LwUnknownDictionary/test new filename", ==, lw_dictionary_get_id (fixture->dictionary));
...
{
  g_assert_cmpint (TOTAL_LW_UNKNOWNDICTIONARYCOLUMNIDS, ==, lw_dictionary_total_columns (fixture->dictionary_class));
}
...
{
    g_assert_cmpint (g_quark_from_string ("unknown"), ==, lw_dictionary_get_column_language (fixture->dictionary_class, LW_UNKNOWNDICTIONARYCOLUMNID_UNKNOWN));
}
...
{
    g_assert_cmpint (LW_DICTIONARYCOLUMNHANDLING_SEARCH_ONLY, ==, lw_dictionary_get_column_handling (fixture->dictionary_class, LW_UNKNOWNDICTIONARYCOLUMNID_UNKNOWN));
}
...
columnid_get_type (Fixture * fixture, gconstpointer data)
{
    ...
    g_assert_true (lw_dictionarymodule_symbol (LW_DICTIONARYMODULE (module), "lw_unknowndictionary_get_columnid_type", (gpointer*) &get_columnid_type));
    ...
    g_assert_cmpuint (klass->n_values, ==, TOTAL_LW_UNKNOWNDICTIONARYCOLUMNIDS);
...
calculate_applicable_columns_for_text_when_english (Fixture * fixture, gconstpointer data)
{
  ...
  g_assert_cmpint (LW_UNKNOWNDICTIONARYCOLUMNID_UNKNOWN, ==, columns[0]);
...
ensure_cache_by_utf8flags (Fixture * fixture, gconstpointer data)
{
...

  {
    ...
    g_assert_cmpstr (lw_parsedline_get_strv (line, LW_UNKNOWNDICTIONARYCOLUMNID_UNKNOWN)[0], ==, "亜 : ｜ 一 口");
  }

  {
    ...
    g_assert_cmpstr (lw_parsedline_get_strv (line, LW_UNKNOWNDICTIONARYCOLUMNID_UNKNOWN)[0], ==, "唖 : ｜ 一 口");
  }

  {
    ...
    g_assert_cmpstr (lw_parsedline_get_strv (line, LW_UNKNOWNDICTIONARYCOLUMNID_UNKNOWN)[0], ==, "娃 : 女 土");
  }

  {
    ...
    g_assert_cmpstr (lw_parsedline_get_strv (line, LW_UNKNOWNDICTIONARYCOLUMNID_UNKNOWN)[0], ==, "阿 : 一 口 亅 阡");
  }

  {
    LwParsedLine * line = lw_parsed_get_line (parsed, 4);
    g_assert_nonnull (line);

    g_assert_cmpstr (lw_parsedline_get_strv (line, LW_UNKNOWNDICTIONARYCOLUMNID_UNKNOWN)[0], ==, "哀 : 衣 口 亠");
  }
