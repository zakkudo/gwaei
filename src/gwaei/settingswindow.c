...
gw_settingswindow_constructed (GObject *object)
{
    ...
    priv->romaji_to_furigana_combobox = GTK_COMBO_BOX (gw_window_get_object (GW_WINDOW (window), "romaji_to_furigana_combobox"));
    priv->furigana_insensitive_checkbutton = GTK_CHECK_BUTTON (gw_window_get_object (GW_WINDOW (window), "furigana_insensitive_checkbutton"));
    priv->case_insensitive_checkbutton = GTK_CHECK_BUTTON (gw_window_get_object (GW_WINDOW (window), "case_insensitive_checkbutton"));
    priv->stem_insensitive_checkbutton = GTK_CHECK_BUTTON (gw_window_get_object (GW_WINDOW (window), "stem_insensitive_checkbutton"));
...
gw_settingswindow_attach_signals (GwSettingsWindow *window)
{
    ...
    priv->signalids[GW_SETTINGSWINDOW_SIGNALID_ROMAJI_TO_FURIGANA] = lw_preferences_add_change_listener_by_schema (
        preferences,
        LW_SCHEMA_BASE,
        LW_KEY_ROMAJI_TO_FURIGANA,
        gw_settingswindow_sync_romaji_to_furigana_cb,
        window
    );

    priv->signalids[GW_SETTINGSWINDOW_SIGNALID_FURIGANA_INSENSITIVE] = lw_preferences_add_change_listener_by_schema (
        preferences,
        LW_SCHEMA_BASE,
        LW_KEY_FURIGANA_INSENSITIVE,
        gw_settingswindow_sync_furigana_insensitive_cb,
        window
    );

    priv->signalids[GW_SETTINGSWINDOW_SIGNALID_CASE_INSENSITIVE] = lw_preferences_add_change_listener_by_schema (
        preferences,
        LW_SCHEMA_BASE,
        LW_KEY_CASE_INSENSITIVE,
        gw_settingswindow_sync_case_insensitive_cb,
        window
    );

    priv->signalids[GW_SETTINGSWINDOW_SIGNALID_STEM_INSENSITIVE] = lw_preferences_add_change_listener_by_schema (
        preferences,
        LW_SCHEMA_BASE,
        LW_KEY_STEM_INSENSITIVE,
        gw_settingswindow_sync_stem_insensitive_cb,
        window
    );
...
gw_settingswindow_remove_signals (GwSettingsWindow *window)
{
    ...
    lw_preferences_remove_change_listener_by_schema (
        preferences, 
        LW_SCHEMA_BASE, 
        priv->signalids[GW_SETTINGSWINDOW_SIGNALID_ROMAJI_TO_FURIGANA]
    );

    lw_preferences_remove_change_listener_by_schema (
        preferences, 
        LW_SCHEMA_BASE, 
        priv->signalids[GW_SETTINGSWINDOW_SIGNALID_FURIGANA_INSENSITIVE]
    );

    lw_preferences_remove_change_listener_by_schema (
        preferences, 
        LW_SCHEMA_BASE, 
        priv->signalids[GW_SETTINGSWINDOW_SIGNALID_CASE_INSENSITIVE]
    );

    lw_preferences_remove_change_listener_by_schema (
        preferences, 
        LW_SCHEMA_BASE, 
        priv->signalids[GW_SETTINGSWINDOW_SIGNALID_STEM_INSENSITIVE]
    );
