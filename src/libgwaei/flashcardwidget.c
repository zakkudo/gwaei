...
gw_flashcardwindow_user_answer_is_correct (GwFlashCardWindow *window)
{
    ...
    //Initializations
    priv = window->priv;
    user = lw_util_normalize_string (gtk_entry_get_text (priv->answer_entry), TRUE, TRUE);
    card = lw_util_normalize_string (priv->answer, TRUE, TRUE);
    convertable = lw_util_str_roma_to_hira (user, hiragana, MAX);

    lw_util_sanitize_string (user);
    lw_util_sanitize_string (card);
