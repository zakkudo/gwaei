...
static void lgw_spellcheck_attach_signals (GwSpellcheck*);
static void lgw_spellcheck_remove_signals (GwSpellcheck*);

G_DEFINE_TYPE (GwSpellcheck, lgw_spellcheck, G_TYPE_OBJECT)
...
GwSpellcheck*
lgw_spellcheck_new (GwApplication *application)
{
...
GwSpellcheck* 
lgw_spellcheck_new_with_entry (GwApplication *application, GtkEntry *entry)
{
    GwSpellcheck *spellcheck;
    
    spellcheck = lgw_spellcheck_new (application);

    if (spellcheck != NULL)
      lgw_spellcheck_set_entry (spellcheck, entry);
...
static void
lgw_spellcheck_init (GwSpellcheck *spellcheck)
{
    spellcheck->priv = GW_SPELLCHECK_GET_PRIVATE (spellcheck);
    memset(spellcheck->priv, 0, sizeof(GwSpellcheckPrivate));

    lgw_spellcheck_set_timeout_threshold (spellcheck, 2);

    lgw_spellcheck_attach_signals (spellcheck);
}


static void
lgw_spellcheck_finalize (GObject *object)
{
    GwSpellcheck *spellcheck;
    ...
    lgw_spellcheck_remove_signals (spellcheck);
    lgw_spellcheck_clear (spellcheck);

    if (priv->entry != NULL) gtk_widget_queue_draw (GTK_WIDGET (priv->entry));

    G_OBJECT_CLASS (lgw_spellcheck_parent_class)->finalize (object);
}


static void 
lgw_spellcheck_set_property (GObject      *object,
                            guint         property_id,
                            const GValue *value,
                            GParamSpec   *pspec)
{
...
static void 
lgw_spellcheck_get_property (GObject      *object,
                            guint         property_id,
                            GValue       *value,
                            GParamSpec   *pspec)
{
...
static void
lgw_spellcheck_class_init (GwSpellcheckClass *klass)
{
    //Declarations
    GParamSpec *pspec;
    ...
    object_class->set_property = lgw_spellcheck_set_property;
    object_class->get_property = lgw_spellcheck_get_property;
    object_class->finalize = lgw_spellcheck_finalize;
...
static void  
lgw_spellcheck_attach_signals (GwSpellcheck *spellcheck)
{
}


static void 
lgw_spellcheck_remove_signals (GwSpellcheck *spellcheck)
{
    //Declarations
    GwSpellcheckPrivate *priv;
    ...
    priv = spellcheck->priv;
    preferences = lgw_application_get_preferences (priv->application);
    entry = priv->entry;
...
void
lgw_spellcheck_set_timeout_threshold (GwSpellcheck *spellcheck, guint threshold)
{
   GwSpellcheckPrivate *priv;
...
void
lgw_spellcheck_set_entry (GwSpellcheck *spellcheck, GtkEntry *entry)
{
    ...
    priv->signalid[GW_SPELLCHECK_SIGNALID_DICTIONARY] = lw_preferences_add_change_listener_by_schema (
        ...
        lgw_spellcheck_sync_dictionary_cb,
        spellcheck
    );
    ...
    priv->signalid[GW_SPELLCHECK_SIGNALID_BUTTON_PRESS_EVENT] = g_signal_connect (
        G_OBJECT (entry), 
        "button-press-event", 
        G_CALLBACK (lgw_spellcheck_button_press_event_cb), 
        spellcheck
    );
    ...
    priv->signalid[GW_SPELLCHECK_SIGNALID_POPULATE_POPUP] = g_signal_connect (
        G_OBJECT (entry), 
        "populate-popup", 
        G_CALLBACK (lgw_spellcheck_populate_popup_cb), 
        spellcheck
    );
    ...
    priv->signalid[GW_SPELLCHECK_SIGNALID_DESTROY] = g_signal_connect_swapped (
        G_OBJECT (entry), 
        "destroy", 
        G_CALLBACK (lgw_spellcheck_remove_signals), 
        spellcheck
    );

    lgw_spellcheck_queue (spellcheck);
}


gboolean
lgw_spellcheck_clear (GwSpellcheck *spellcheck)
{
...
gint
lgw_spellcheck_get_layout_y_offset (GwSpellcheck *spellcheck)
{
...
gint
lgw_spellcheck_get_layout_x_offset (GwSpellcheck *spellcheck)
{
    g_return_val_if_fail (spellcheck != NULL, 0);
...
gboolean
lgw_spellcheck_has_hiragana_conversion (GwSpellcheck *spellcheck)
{
    //Declarations
    GwSpellcheckPrivate *priv;
    const gint MAX = 300;
...
gboolean
lgw_spellcheck_should_check (GwSpellcheck *spellcheck)
{
    //Declarations
    ...
    //Initializations
    priv = spellcheck->priv;
    has_hiragana_conversion = lgw_spellcheck_has_hiragana_conversion (spellcheck);
    query = gtk_entry_get_text (priv->entry);
    engine = lgw_application_get_morphologyengine (priv->application);
...
void
lgw_spellcheck_queue (GwSpellcheck *spellcheck)
{
    ...
    //Initializations
    priv = spellcheck->priv;
    should_check = lgw_spellcheck_should_check (spellcheck);
    engine = lgw_application_get_morphologyengine (priv->application);
    ...
    if (should_check)
    {
      ...
      if (priv->timeoutid[GW_SPELLCHECK_TIMEOUTID_UPDATE] == 0)
        priv->timeoutid[GW_SPELLCHECK_TIMEOUTID_UPDATE] = g_timeout_add_full (
          G_PRIORITY_LOW, 
          100, (GSourceFunc) 
          lgw_spellcheck_update_timeout, 
          spellcheck, 
          NULL
      );
    }

    should_redraw = lgw_spellcheck_clear (spellcheck);
    if (should_redraw) 
    {
...
static gboolean
lgw_spellcheck_is_common_script (const gchar *TEXT)
{
...
gboolean
lgw_spellcheck_update (GwSpellcheck *spellcheck)
{
    //Sanity checks
    g_return_val_if_fail (spellcheck != NULL, FALSE);
    ...
    //Initializations
    priv = spellcheck->priv;
    morphologyengine = lgw_application_get_morphologyengine (priv->application);
    ...
    should_check = lgw_spellcheck_should_check (spellcheck);
    should_redraw = lgw_spellcheck_clear (spellcheck); //Make sure the memory is freed
    ...
    if (should_check)
    {
      ...
      {
        if (**iter != '\0' && lgw_spellcheck_is_common_script (*iter))
        {
...
void
lgw_spellcheck_record_mouse_cordinates (GwSpellcheck *spellcheck, 
                                       GdkEvent     *event)
{
    GwSpellcheckPrivate *priv;
    GtkWidget *toplevel;
...
void 
lgw_spellcheck_populate_popup (GwSpellcheck *spellcheck, 
                              GtkMenu      *menu)
{
    //Sanity checks
    g_return_if_fail (spellcheck != NULL);
    ...
    xoffset = lgw_spellcheck_get_layout_x_offset (spellcheck);
    yoffset = lgw_spellcheck_get_layout_y_offset (spellcheck);
    x = priv->x - xoffset;
    ...
    if (morphology->spellcheck != NULL)
    {
      ...
      if (total_suggestions > 0 && suggestions != NULL)
      {
        ...
        if (text != NULL)
        {
          ...
          g_signal_connect (G_OBJECT (menuitem), "activate", G_CALLBACK (lgw_spellcheck_add_menuitem_activated_cb), spellcheck);
        ...
        for (i = 0; i < total_suggestions; i++)
        {
          ...
          g_signal_connect (G_OBJECT (menuitem), "activate", G_CALLBACK (lgw_spellcheck_menuitem_activated_cb), spellcheck);
