...
static void _kanjipadwindow_draw_candidate_character (GwKanjipadWindow *window, int index, int selected)
{
    ...
    gtk_style_context_get_color (context, state, &fgcolorn);
    gtk_style_context_get_background_color (context, state, &bgcolorn);
    gtk_style_context_get_color (context, state | GTK_STATE_FLAG_SELECTED, &fgcolors);
    gtk_style_context_get_background_color (context, state | GTK_STATE_FLAG_SELECTED, &bgcolors);
    ...
    {
      if (selected)
        gdk_cairo_set_source_rgba (cr, &bgcolors);
      else
        gdk_cairo_set_source_rgba (cr, &bgcolorn);
    ...
    if (selected >= 0 && selected)
      gdk_cairo_set_source_rgba (cr, &fgcolors);
    else
      gdk_cairo_set_source_rgba (cr, &fgcolorn);
