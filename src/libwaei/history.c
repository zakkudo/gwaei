...
{ 
    ...
    priv->back = g_list_prepend (priv->back, iterator);
...
//! @param iterator The LwSearch to check for history relevance
...
{
    //Sanity checks
    if (iterator == NULL || iterator->search == NULL) return FALSE;
