 ...
 *
 * |[<!-- langauge="c" -->
 * LwDictionaryModuleReader * reader = lw_dictionary_module_reader_open (NULL);
 * if (reader != NULL)
 * {
 *   gchar const * path = NULL;
 *   GList * modules = NULL;
 *   while ((path = lw_dictionary_module_reader_read_path (reader)) != NULL)
 *   {
 *     modules = g_list_prepend (modules, lw_dictionary_module_new (path));
 *   }
 *   lw_dictionary_module_reader_close (reader);
 * }
 * ]|
 */
...
G_DEFINE_TYPE_WITH_PRIVATE (LwDictionaryModule, lw_dictionary_module, G_TYPE_TYPE_MODULE)


/**
 * lw_dictionary_module_new:
 ...
 */
GTypeModule *
lw_dictionary_module_new (gchar const * PATH)
{
    //Declarations
    GTypeModule *self = NULL;

    //Initializations
    self = G_TYPE_MODULE (g_object_new (LW_TYPE_DICTIONARY_MODULE, "path", PATH, NULL));
...
static void
lw_dictionary_module_init (LwDictionaryModule * self)
{
    LwDictionaryModulePrivate *priv = NULL;

    priv = lw_dictionary_module_get_instance_private (self);
}


static void 
lw_dictionary_module_set_property (GObject      * object,
                                  guint          property_id,
                                  const GValue * value,
                                  GParamSpec   * pspec)
{
    ...
    //Initializations
    self = LW_DICTIONARY_MODULE (object);
    priv = lw_dictionary_module_get_instance_private (self);

    switch (property_id)
    {
      case PROP_PATH:
        lw_dictionary_module_set_path (self, g_value_get_string (value));
        break;
      default:
...
static void 
lw_dictionary_module_get_property (GObject    * object,
                                  guint        property_id,
                                  GValue     * value,
                                  GParamSpec * pspec)
{
    ...
    {
      case PROP_PATH:
        g_value_set_string (value, lw_dictionary_module_get_path (self));
        break;
      case PROP_NAME:
        g_value_set_string (value, lw_dictionary_module_get_name (self));
        break;
      default:
        G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
        break;
...
static void 
lw_dictionary_module_finalize (GObject * object)
{
    //Declarations
    LwDictionaryModule *self = NULL;
    LwDictionaryModulePrivate *priv = NULL;
    ...
    G_OBJECT_CLASS (lw_dictionary_module_parent_class)->finalize (object);
}


static void
lw_dictionary_module_dispose (GObject * object)
{
    //Declarations
    LwDictionaryModule *self = NULL;
    ...
    G_OBJECT_CLASS (lw_dictionary_module_parent_class)->dispose (object);
}


static void
lw_dictionary_module_class_init (LwDictionaryModuleClass * klass)
{
    //Declarations
    ...
    object_class->set_property = lw_dictionary_module_set_property;
    object_class->get_property = lw_dictionary_module_get_property;
    object_class->dispose = lw_dictionary_module_dispose;
    object_class->finalize = lw_dictionary_module_finalize;

    module_class = G_TYPE_MODULE_CLASS (klass);
    module_class->load = lw_dictionary_module_load;
    module_class->unload = lw_dictionary_module_unload;
...
/**
 * lw_dictionary_module_get_dictionarylib_searchpath:
 ...
 */
gchar **
lw_dictionary_module_get_dictionarylib_searchpath (gchar const * SEARCHPATH_OVERRIDE)
{
    //Declarations
...
static void
lw_dictionary_module_open (LwDictionaryModule *self)
{
    //Sanity checks
    g_return_if_fail (LW_IS_DICTIONARY_MODULE (self));
...
static void
lw_dictionary_module_close (LwDictionaryModule *self)
{
    //Sanity checks
    g_return_if_fail (LW_IS_DICTIONARY_MODULE (self));
...
static gboolean 
lw_dictionary_module_register_type (LwDictionaryModule *self)
{
    //Sanity checks
    g_return_val_if_fail (LW_IS_DICTIONARY_MODULE (self), FALSE);
...
static gboolean
lw_dictionary_module_load (GTypeModule *module)
{
    //Sanity checks
    g_return_val_if_fail (LW_IS_DICTIONARY_MODULE (module), FALSE);

    //Declarations
    LwDictionaryModule *self = NULL;

    //Initializations
    self = LW_DICTIONARY_MODULE (module);

    g_assert_true (g_module_supported ());

    lw_dictionary_module_open (self);

errored:

    return lw_dictionary_module_register_type (self);
}


static void
lw_dictionary_module_unload (GTypeModule *module)
{
    //Sanity checks
    g_return_if_fail (LW_IS_DICTIONARY_MODULE (module));
    ...
    lw_dictionary_module_close (self);
}


/**
 * lw_dictionary_module_get_name:
 ...
 */
gchar const *
lw_dictionary_module_get_name (LwDictionaryModule * self)
{
    //Sanity checks
    g_return_val_if_fail (LW_IS_DICTIONARY_MODULE (self), NULL);
...
static void
lw_dictionary_module_sync_name (LwDictionaryModule * self)
{
    //Sanity checks
    g_return_if_fail (LW_IS_DICTIONARY_MODULE (self));
    ...
    //Initializations
    priv = lw_dictionary_module_get_instance_private (self);
    klass = LW_DICTIONARY_MODULE_GET_CLASS (self);
 ...
 */
void
lw_dictionary_module_set_path (LwDictionaryModule * self,
                              gchar const        * PATH)
{
    //Sanity checks
    g_return_if_fail (LW_IS_DICTIONARY_MODULE (self));
    ...
    lw_dictionary_module_sync_name (self);
 ...
 */
gchar const *
lw_dictionary_module_get_path (LwDictionaryModule * self)
{
    //Sanity checks
    g_return_val_if_fail (LW_IS_DICTIONARY_MODULE (self), NULL);
 ...
 */
gboolean
lw_dictionary_module_symbol (LwDictionaryModule * self,
...
/**
 * lw_dictionary_module_reader_open:
 ...
 * 
 * Returns: (transfer full): A new #LwDictionaryModuleReader that should be closed with lw_dictionary_module_reader_close()
 */
LwDictionaryModuleReader *
lw_dictionary_module_reader_open (gchar const * SEARCHPATH_OVERRIDE)
{
    LwDictionaryModuleReader * self = NULL;
    gchar ** paths = NULL;

    paths = lw_dictionary_module_get_dictionarylib_searchpath (SEARCHPATH_OVERRIDE);
...
/**
 * lw_dictionary_module_reader_read_path:
 * @self: A #LwDictionaryModuleReader
 *
 ...
 */
gchar *
lw_dictionary_module_reader_read_path (LwDictionaryModuleReader * self)
{
    //Sanity checks
